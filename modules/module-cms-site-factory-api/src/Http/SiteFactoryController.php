...
final class SiteFactoryController
{
    ...
    public function update(Request $request, int $id, SiteFactoryService $service): JsonResponse
    {
        ...
        $data = $this->validated($request, ['name' => ['sometimes', 'string', 'max:255'], 'status' => ['sometimes', 'string', 'in:active,suspended,archived']]);
        if (isset($data['status'])) {
            $service->transition($site, $this->stringInput($data, 'status'));
        }
        if (isset($data['name'])) {
            $site->forceFill(['name' => $this->stringInput($data, 'name')])->save();
        }

        return response()->json(['data' => $this->site($site->refresh())]);
    }

    public function destroy(int $id, SiteFactoryService $service): JsonResponse
    {
        $service->archive(Site::query()->findOrFail($id));

        return response()->json(status: 204);
    }

    public function templates(SiteFactoryService $service): JsonResponse
    {
        return response()->json(['data' => $service->templates()->map(fn (SiteTemplate $template): array => $this->templateData($template))->values()->all()]);
    }
    ...
    public function cloneById(Request $request, int $id, SiteFactoryService $service): JsonResponse
    {
        $data = $this->validated($request, ['key' => ['required', 'string', 'max:100'], 'name' => ['required', 'string', 'max:255']]);

        return response()->json(['data' => $this->site($service->clone(Site::query()->findOrFail($id), $this->stringInput($data, 'key'), $this->stringInput($data, 'name')))], 201);
    }

    public function suspendById(int $id, SiteFactoryService $service): JsonResponse
    {
        return response()->json(['data' => $this->site($service->suspend(Site::query()->findOrFail($id)))]);
    }

    public function archiveById(int $id, SiteFactoryService $service): JsonResponse
    {
        return response()->json(['data' => $this->site($service->archive(Site::query()->findOrFail($id)))]);
    }

    public function teardownById(Request $request, int $id, SiteFactoryService $service): JsonResponse
    {
        $service->teardown(Site::query()->findOrFail($id), $request->boolean('confirm'));
    ...
    {
        return $this->suspendById($this->siteId($site), $service);
    }

    public function archive(string $site, SiteFactoryService $service): JsonResponse
    {
        return $this->archiveById($this->siteId($site), $service);
    }

    public function clone(Request $request, string $site, SiteFactoryService $service): JsonResponse
    {
        return $this->cloneById($request, $this->siteId($site), $service);
    }

    public function verifyDomain(Request $request, int $domain, SiteFactoryService $service): JsonResponse
    {
        $data = $this->validated($request, ['token' => ['required', 'string']]);

        return response()->json(['data' => $this->domain($service->verifyDomain(SiteDomain::query()->findOrFail($domain), $this->stringInput($data, 'token')))]);
    }

    public function teardown(Request $request, string $site, SiteFactoryService $service): JsonResponse
    {
        return $this->teardownById($request, $this->siteId($site), $service);
    }

    /** @return array<string, mixed> */
    private function site(Site $site): array
    {
        return $site->only(['id', 'key', 'name', 'domain', 'default_locale', 'timezone', 'status', 'settings', 'team_id', 'created_at', 'updated_at']);
    }

    /** @return array<string, mixed> */
    private function templateData(SiteTemplate $template): array
    {
        return $template->only(['id', 'key', 'name', 'configuration', 'initial_content', 'active', 'team_id', 'created_at', 'updated_at']);
    }

    /** @return array<string, mixed> */
    private function domain(SiteDomain $domain): array
    {
        return $domain->only(['id', 'site_id', 'domain', 'verified_at', 'team_id', 'created_at', 'updated_at']);
    }

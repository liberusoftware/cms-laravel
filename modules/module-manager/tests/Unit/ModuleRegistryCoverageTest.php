...
it('exposes and normalizes all manifest metadata', function (): void {
...
it('rejects malformed manifests', function (array $mutation, string $message): void {
    $data = [
        'name' => 'example', 'display_name' => 'Example', 'description' => 'Test',
        'version' => '1.0.0', 'category' => 'capability',
        'provider' => ServiceProvider::class,
    ...
    mkdir($dir, 0777, true);
    file_put_contents($dir.'/module.json', json_encode($data, JSON_THROW_ON_ERROR));
    expect(fn (): Manifest => Manifest::fromFile($dir.'/module.json'))->toThrow(InvalidManifest::class, $message);
})->with([
...
it('loads writes and clears a valid registry cache and rejects invalid cache data', function (): void {
    ...
    expect(fn (): ModuleRegistry => $cache->load([], true, $path))->toThrow(RuntimeException::class, 'cache is invalid');
});

it('reports registry cache filesystem write and delete failures', function (): void {
...
it('rejects every invalid module discovery collision', function (Closure $arrange, string $message): void {
    $root = sys_get_temp_dir().'/discovery-'.bin2hex(random_bytes(5));
    mkdir($root, 0777, true);
    $arrange($root);
    expect(fn (): ModuleRegistry => (new ModuleDiscovery)->discover([$root]))->toThrow(InvalidManifest::class, $message);
})->with([
    ...
    'missing package name' => [function (string $root): void {
    ...
    'duplicate module' => [function (string $root): void {
    ...
    'duplicate package' => [function (string $root): void {
        foreach (['one', 'two'] as $name) {
            $manifest = makeCoverageManifest(['name' => $name], 'local/shared');
            rename($manifest->path, $root.'/'.$name);
        }
    }, 'missing or duplicate Composer package name'],
    'duplicate capability' => [function (string $root): void {
        foreach (['one', 'two'] as $name) {
...
it('reports all invalid module metadata and dependency errors', function (): void {
    $manifest = makeCoverageManifest([
        'name' => 'invalid',
        'version' => '999.0.0',
...
it('reports an installed Composer version mismatch and a non-provider class', function (): void {
    $manifest = makeCoverageManifest([
...
it('orders dependencies and exposes lookup and enabled state', function (): void {
...
it('deduplicates identical installed and tracked module copies', function (): void {
    $roots = [];
...
it('rejects a disabled capability provider', function (): void {
    ...
    expect(fn (): array => new ModuleRegistry(['owner' => $owner, 'consumer' => $consumer])->resolve([]))

...
final class SitemapsController
{
    ...
    public function create(Request $request, SitemapService $service): JsonResponse
    {
        $data = $request->validate(['url' => ['required', 'url', 'max:2048'], 'site_id' => ['sometimes', 'nullable', 'integer'], 'type' => ['sometimes', 'string', 'max:50'], 'locale' => ['sometimes', 'nullable', 'string', 'max:20'], 'priority' => ['sometimes', 'numeric', 'between:0,1'], 'extensions' => ['sometimes', 'array']]);

        return response()->json(['data' => SitemapEntryResource::make($service->add($data['url'], $data['site_id'] ?? null, $data['type'] ?? 'web', $data['locale'] ?? null, (float) ($data['priority'] ?? .5), $data['extensions'] ?? []))], 201);
    }

    public function exclude(Request $request, SitemapService $service): JsonResponse
    {
    ...
    {
        $data = $request->validate(['engine' => ['required', 'in:google,bing'], 'site_id' => ['sometimes', 'nullable', 'integer']]);

        return response()->json(['data' => $service->notify($data['engine'], $data['site_id'] ?? null)], 202);
    }

    public function chunks(Request $request, SitemapService $service): JsonResponse
    {
        $data = $request->validate(['site_id' => ['sometimes', 'nullable', 'integer'], 'type' => ['sometimes', 'nullable', 'string'], 'locale' => ['sometimes', 'nullable', 'string'], 'size' => ['sometimes', 'integer', 'min:1', 'max:50000']]);

        return response()->json(['data' => array_map(fn (array $chunk): array => array_map(SitemapEntryResource::make(...), $chunk), $service->chunks($data['site_id'] ?? null, (int) ($data['size'] ?? 50000), $data['type'] ?? null, $data['locale'] ?? null))]);
    }
}

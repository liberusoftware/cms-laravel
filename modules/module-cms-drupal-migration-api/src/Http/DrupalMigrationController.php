...
namespace Liberu\Cms\DrupalMigrationApi\Http;
...
final class DrupalMigrationController
{
    public function index(Request $request): JsonResponse
    {
        return response()->json(['data' => MigrationJob::query()->where('source', 'drupal')->latest()->paginate(max(1, min(100, $request->integer('per_page', 15))))]);
    }

    public function start(Request $request, DrupalMigrationService $service): JsonResponse
    {
    ...
    {
        return MigrationJob::query()->where('public_id', $publicId)->where('source', 'drupal')->firstOrFail();
    }

    public function add(string $publicId, Request $request, DrupalMigrationService $service): JsonResponse
    {
    ...
    public function process(string $publicId, int|string $record, Request $request, DrupalMigrationService $service): JsonResponse
    {
    ...
    public function complete(string $publicId, DrupalMigrationService $service): JsonResponse
    {

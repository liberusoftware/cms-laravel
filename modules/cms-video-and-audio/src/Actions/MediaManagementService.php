...
final class MediaManagementService
{
    ...
    {
        if (! in_array($attributes['kind'] ?? null, config('video-and-audio.asset_kinds', []), true)) throw ValidationException::withMessages(['kind' => 'Choose video or audio.']);
        if (! in_array($attributes['source_type'] ?? null, ['upload', 'remote'], true) || blank($attributes['source_uri'] ?? null)) throw ValidationException::withMessages(['source_uri' => 'A valid upload or remote source is required.']);
        if (($attributes['source_type'] ?? null) === 'remote' && filter_var($attributes['source_uri'], FILTER_VALIDATE_URL) === false) throw ValidationException::withMessages(['source_uri' => 'Remote sources must be valid URLs.']);
        if (isset($attributes['bytes']) && (int) $attributes['bytes'] > (int) config('video-and-audio.max_upload_bytes', 5368709120)) throw ValidationException::withMessages(['bytes' => 'The media source exceeds the configured size limit.']);
    ...
    public function addTrack(MediaAsset $asset, array $attributes): MediaTrack
    {
        ...
        if (! in_array($type, config('video-and-audio.track_types', []), true)) throw ValidationException::withMessages(['track_type' => 'Unsupported media track type.']);
        if ($type === 'chapter' && ((float) ($attributes['end_seconds'] ?? 0) <= (float) ($attributes['start_seconds'] ?? 0))) throw ValidationException::withMessages(['end_seconds' => 'Chapter end must be after its start.']);
        if (in_array($type, ['caption', 'transcript'], true) && blank($attributes['uri'] ?? $attributes['content'] ?? null)) throw ValidationException::withMessages(['content' => 'Captions and transcripts require a URI or content.']);
    ...
    {
        if ($asset->status === 'archived') throw ValidationException::withMessages(['status' => 'Archived media cannot be updated.']);
        if (array_key_exists('source_uri', $attributes) && blank($attributes['source_uri'])) throw ValidationException::withMessages(['source_uri' => 'A media source is required.']);
    ...
    {
        if ($asset->status === 'archived') throw ValidationException::withMessages(['status' => 'Archived media cannot be transcoded.']);
        if (trim($idempotencyKey) === '') throw ValidationException::withMessages(['idempotency_key' => 'An idempotency key is required.']);
    ...
    {
        if ($asset->status !== 'ready') throw ValidationException::withMessages(['status' => 'Media is not ready for playback.']);

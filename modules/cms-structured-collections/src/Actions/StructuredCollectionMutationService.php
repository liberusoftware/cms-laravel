...
final readonly class StructuredCollectionMutationService
{
    ...
    {
        return $this->legacy->create($attributes);
    }

    public function update(Collection $collection, array $attributes): Collection
    {
        return $this->legacy->update($collection, $attributes);
    }

    public function delete(Collection $collection): void
    {
        $this->legacy->delete($collection);
    }

    public function createRecord(Collection $collection, array $attributes): CollectionItem
    {
        return $this->legacy->createItem($collection, $attributes);
    }

    public function updateRecord(CollectionItem $record, array $attributes): CollectionItem
    {
        return $this->legacy->updateItem($record, $attributes);
    }

    public function deleteRecord(CollectionItem $record): void
    {
        $this->legacy->deleteItem($record);
    }
}

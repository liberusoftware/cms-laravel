...
final class WordPressMigrationService
{
    ...
    {
        if ($sourceUrl !== null && filter_var($sourceUrl, FILTER_VALIDATE_URL) === false) throw ValidationException::withMessages(['source_url' => 'Source URL must be valid.']);
    ...
    {
        if (! in_array($recordType, self::TYPES, true) || trim($sourceId) === '') throw ValidationException::withMessages(['record_type' => 'Unsupported record type or missing source identifier.']);
        if (! in_array($migration->status, ['draft', 'running'], true)) throw ValidationException::withMessages(['migration' => 'Records cannot be added after migration completion.']);
    ...
    {
        if (! $success && trim((string) $failureReason) === '') throw ValidationException::withMessages(['failure_reason' => 'A failed record requires a reason.']);
        return DB::transaction(function () use ($record, $success, $failureReason): WordPressMigrationRecord {
            ...
            if (! $wasProcessed) $record->migration()->increment($success ? 'processed_records' : 'failed_records');
            return $record->refresh();
        });
    }
    ...
    {
        if (trim($reason) === '') throw ValidationException::withMessages(['failure_reason' => 'Failure reason is required.']);

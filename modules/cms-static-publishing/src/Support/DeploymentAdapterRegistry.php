...
final class DeploymentAdapterRegistry
{
    ...
    public function register(DeploymentAdapterInterface $adapter): void { $this->adapters[$adapter->key()] = $adapter; }

    public function resolve(string $key): DeploymentAdapterInterface
    {
        if (! isset($this->adapters[$key])) throw ValidationException::withMessages(['deployment' => "Unknown deployment adapter: {$key}."]);

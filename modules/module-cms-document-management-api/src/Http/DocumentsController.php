...
final class DocumentsController
{
    ...
    {
        return response()->json(['data' => $service->create($request->validate(['title' => ['required', 'string'], 'slug' => ['required', 'string'], 'path' => ['nullable', 'string'], 'mime_type' => ['nullable', 'string'], 'size' => ['nullable', 'integer', 'min:0']]), $request->user()?->current_team_id)], 201);
    }

    public function status(Request $request, Document $document, DocumentManagementService $service): JsonResponse
    {
        return response()->json(['data' => $service->transition($document, $request->validate(['status' => ['required', 'in:draft,processing,ready,archived,failed']])['status'])]);
    }
}

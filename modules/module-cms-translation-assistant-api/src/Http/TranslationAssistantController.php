...
final class TranslationAssistantController
{
    ...
    public function show(int|string $draft, TranslationAssistantQuery $query): JsonResponse
    {
        $model = $query->draft($draft);
        abort_unless($model, 404);

        return response()->json(['data' => TranslationDraftResource::make($model)]);
    }
    ...
    public function review(Request $request, int|string $draft, TranslationAssistantQuery $query, TranslationAssistantService $service): JsonResponse
    {
        $model = $query->draft($draft);
        abort_unless($model, 404);
        $data = $request->validate(['decision' => ['required', 'in:approved,rejected'], 'reviewer_type' => ['required', 'string', 'max:100'], 'reviewer_id' => ['required', 'string', 'max:100']]);

        return response()->json(['data' => TranslationDraftResource::make($service->review($model, $data['decision'], $data['reviewer_type'], $data['reviewer_id']))]);
    }
    ...
    public function check(int|string $draft, TranslationAssistantQuery $query, TranslationAssistantService $service): JsonResponse
    {
        ...
        return response()->json(['data' => TranslationDraftResource::make($service->check($model))]);
    }
